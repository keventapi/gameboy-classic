class InstructionMixin:
    ...
    def ld_r1_r2(self, decoded):
        if len(decoded[1]) > 1:
            if decoded[1] == "nn":
                ...
                self.mmu.write(nn, self.registers[decoded[2]])
                return None
            
            reg16 = (self.registers[decoded[1][0]] << 8) | self.registers[decoded[1][1]]
            self.mmu.write(reg16, self.registers[decoded[2]])
        ...
        # caso r2 de 16 bits
            ...
            self.registers[decoded[1]] = self.mmu.read(reg16)
    ...
    def ld_a_FF00_C(self, decoded):
        ...
        if instruction == 0:
            self.registers["A"] = self.mmu.read(0xFF00 + self.registers["C"])
        else:
            self.mmu.write(0xFF00 + self.registers["C"], self.registers["A"])
    
    def ldf_x_yz(self, decoded):
        if decoded[0][6] != " ":
            r1 = decoded[4]
            r2 = decoded[5]
            yz = (self.registers[r1] << 8) | self.registers[r2]
            x = decoded[0][-1]
            self.mmu.write(yz, self.registers[x])
        else:
            r1 = decoded[-2]
            r2 = decoded[-1]
            yz = (self.registers[r1] << 8) | self.registers[r2]
            x = decoded[4]
            self.registers[x] = self.mmu.read(yz)
